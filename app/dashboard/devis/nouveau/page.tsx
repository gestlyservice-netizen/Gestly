...
export default function NouveauDevisPage() {
  ...
  const totalTVA = (watchedLines ?? []).reduce((acc, l, i) =>
    acc + lineTotals[i] * (Number(l?.tvaRate) || 0) / 100, 0
  );
  ...
  const submitDevis = async (data: any) => {
    ...
    try {
      const res = await fetch("/api/devis", {
        ...
        body: JSON.stringify({ ...data, status: pendingStatus.current }),
      });
      if (res.ok) {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">Le numéro sera généré automatiquement</p>
        </div>
      </div>
      ...
      <div className="space-y-5">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          ...
          <button
            type="button"
            onClick={() =>
              append({ description: "", quantity: 1, unitPriceHT: 0, tvaRate: 20 })
            }
        ...
        <div className="flex flex-col sm:flex-row justify-end gap-3 pb-6">
          ...
          <button
            ...
            disabled={saving}
            onClick={() => {
              pendingStatus.current = "brouillon";
              handleSubmit(submitDevis)();
            }}
            className="inline-flex items-center justify-center gap-2 border border-slate-300 hover:border-slate-400 text-slate-700 text-sm font-semibold px-6 py-2.5 rounded-lg transition-colors disabled:opacity-60"
          >
            {saving && pendingStatus.current === "brouillon" && (
          ...
          >
            {saving && pendingStatus.current === "envoye" && (
              <Loader2 className="h-4 w-4 animate-spin" />
            )}
